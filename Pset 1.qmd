...
determine_poker_hand <- function(suits, ranks) {
  # Sort ranks in descending order and count unique ranks
  ...
  if (length(unique(suits)) == 1 && all(diff(sort(ranks)) == 1)) { # the unique function is used to eliminate or delete the duplicate values.
    return("Straight Flush")
  } else if (any(sort_ranks == 4)) { 
    return("Four of a Kind")
  } else if (any(sort_ranks == 3) && any(sort_ranks == 2)) {
    return("Full House")
...
deal_poker_round <- function(num_players) {
  ...
  # the expand.grid function is used to create a data frame from all combinations of the supplied vectors.
